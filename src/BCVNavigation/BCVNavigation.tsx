...
const findBookInNavigableBooksByBookNumber = (
  ...
  bookNumber?: number
): NavigableBook | null | undefined =>
  bookNumber
    ? navigableBooks.find((book) => book.BookNumber === bookNumber)
    : null;
...
const BCVNavigation = ({
  ...
  const [booksWithNavigationInfo, setBooksWithNavigationInfo] = useState(
    [] as NavigableBook[]
  );
  const [selectedBook, setSelectedBook] = useState(
    null as NavigableBook | null | undefined
  );
  ...
  const availableVerses = useMemo(
    () =>
      availableChapters && selectedChapter && selectedChapter !== NO_VALUE
        ? availableChapters.find(
            (chapter) => chapter.reference === selectedChapter
          )?.verses
        : undefined,
  ...
  const handleNavigation = useMemo(
    () => () =>
      ...
      selectedVerse &&
      onNavigate(
        new BCVWP(selectedBook.BookNumber, selectedChapter, selectedVerse)
      ),
    ...
    <>
      ...
      <label aria-disabled={disabled}>
        ...
        <select
          ...
          onChange={(e) =>
            setSelectedVerse(
              e.target.value ? Number(e.target.value) : (NO_VALUE as number)
            )
          }
        >
          <option key={NO_VALUE} value={NO_VALUE}>
